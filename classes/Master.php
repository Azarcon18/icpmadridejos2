...
// `, `date_created`, `date_updated`, `phone_no`,
	...
	function save_baptism_req() {
		...
		$columns = array(
			...
			'date_issue', 'status' // Include the status field
		);
		$data = [];
		...
		if (empty($id)) {
			// INSERT new record
			$sql = "INSERT INTO `baptism_request` (`" . implode("`, `", array_keys($data)) . "`) VALUES ('" . implode("', '", $data) . "')";
		} else {
			...
			$sql = "UPDATE `baptism_request` SET " . implode(", ", $update_fields) . " WHERE id = '{$id}'";
		}
		...
		if ($save) {
			$resp['status'] = 'success';
			// Optional: You can return the ID of the inserted/updated record
			if (!empty($id)) {
				$resp['id'] = $id;
			} else {
				$resp['id'] = $this->conn->insert_id; // Get the last inserted ID for new records
			}
	...
	function delete_baptism_request(){
		...
		$save = $this->conn->query("DELETE FROM `baptism_request` WHERE id = '{$id}'");
		if($save){
			$resp['status'] = 'success';
			$this->settings->set_flashdata("success","Baptism Request Successfully Deleted.");
		}else{
			$resp['status'] = 'failed';
			$resp['err'] = $this->conn->error;
		}
	...
	function save_wedding_req() {
		...
		$columns = array(
			...
			'birthdate', 'age', 'birthplace', 'gender', 'citizenship', 'address', 
			...
			'wife_gender', 'wife_citizenship', 'wife_address', 'wife_civil_status', 
		...
		if (empty($id)) {
			// INSERT new record
			$sql = "INSERT INTO `wedding_request` (`" . implode("`, `", array_keys($data)) . "`) VALUES ('" . implode("', '", $data) . "')";
		} else {
			// UPDATE existing record
			$update_fields = [];
			foreach ($data as $key => $value) {
			...
			$sql = "UPDATE `wedding_request` SET " . implode(", ", $update_fields) . " WHERE id = '{$id}'";
		}
	...
	function delete_wedding_request() {
		...
		// Execute delete query
		$save = $this->conn->query("DELETE FROM `wedding_request` WHERE id = '{$id}'");
		if ($save) {
			$resp['status'] = 'success';
